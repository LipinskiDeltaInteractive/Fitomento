...
if(isset($_POST['wysylka']) && $_POST['wysylka']!=""){
	$filters[]='b.order_shipping_id = '.(int)$_POST['wysylka'];
}
if(isset($_POST['wysylka']) && $_POST['wysylka']!=""){
	$filters[]='b.order_shipping_id = '.(int)$_POST['wysylka'];
}
		...
		$pageInfo->filter->filter_product=$_POST['produkt_id'];
...
if(isset($_POST['produkt_id']) && $_POST['produkt_id']!=""){
		//$tables = ' INNER JOIN '.hikashop_table('order_product').' AS p ON b.order_id = p.order_id ';
		//$filters[] = ' p.product_id="'.$_POST['produkt_id'].'"';
		}
...
//echo $query; 
	...
	function export() {
		...
		if(!empty($rows)) {
			...
			if(!empty($addressIds)) {
				$database->setQuery('SELECT * , address_id as adres_do_wysylki_kod_paczkomatu, address_id as adres_do_wysylki_adres_do_paczkomatu, address_id as adres_do_wysylki_imie_nazwisko, address_id as adres_do_wysylki_adres,address_id as adres_do_wysylki_kod_pocztowy, address_id as adres_do_wysylki_miasto, address_id as adres_do_wysylki_telefon FROM '.hikashop_table('address').' WHERE address_id IN ('.implode(',',$addressIds).')');
				...
				if(!empty($addresses)) {
					...
					foreach($addresses as $i => $address) {
								...
								$addresses[$i]->adres_do_wysylki_kod_paczkomatu= substr($addresses[$i]->adres_paczkomatu, 0, strpos($addresses[$i]->adres_paczkomatu, " ")); 
								$addresses[$i]->adres_do_wysylki_adres_do_paczkomatu= substr($addresses[$i]->adres_paczkomatu, strpos($addresses[$i]->adres_paczkomatu, " ")); 
								
								
								
									if($addresses[$i]->innyadres!=""){ 
									
									
									
									
									$addresses[$i]->adres_do_wysylki_imie_nazwisko=$addresses[$i]->imiinazwisko; 
									$addresses[$i]->adres_do_wysylki_adres=$addresses[$i]->ulicainumer; 
									$addresses[$i]->adres_do_wysylki_kod_pocztowy=$addresses[$i]->kod_pocztowy_inny;
									$addresses[$i]->adres_do_wysylki_miasto=$addresses[$i]->miejscowosc_inny;
									$addresses[$i]->adres_do_wysylki_telefon=$addresses[$i]->telefon_inny;
									
									}else{ 
									$addresses[$i]->adres_do_wysylki_imie_nazwisko=$addresses[$i]->address_firstname.' '.$addresses[$i]->address_lastname;
									$addresses[$i]->adres_do_wysylki_adres=$addresses[$i]->address_street; 
									$addresses[$i]->adres_do_wysylki_kod_pocztowy=$addresses[$i]->address_post_code;
									$addresses[$i]->adres_do_wysylki_miasto=$addresses[$i]->address_city;
									$addresses[$i]->adres_do_wysylki_telefon=$addresses[$i]->address_telephone;
									}
