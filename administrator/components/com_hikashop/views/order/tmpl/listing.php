...
<?php 
$db = & JFactory::getDBO();
	$sql = "SELECT product_id, product_name FROM #__hikashop_product where product_published=1";
	$db->setQuery($sql);
	$produkty = $db->loadObjectList();
	
	?>
	<select onChange="this.form.submit();" name="produkt_id">
	<option value="">wybierz produkt</option>
	<?php foreach($produkty as $produkty2){ ?>
	<option <?php if($_POST['produkt_id']==$produkty2->product_id){ ?> selected <?php } ?> value="<?php echo $produkty2->product_id; ?>"><?php  echo $produkty2->product_name;?></option>
<?php } ?>
...
<select onChange="document.getElementById('adminForm').submit();" name="wysylka">
<option value="">wybierz</option>
<option <?php if(isset($_POST['wysylka']) && $_POST['wysylka']=="2"){ ?> selected <?php } ?> value="2">Paczkomaty</option>
<option <?php if(isset($_POST['wysylka']) && $_POST['wysylka']=="3"){ ?> selected <?php } ?> value="3">Kurier InPost</option>
</select>
		...
		<thead>
			<tr>
				...
				<th class="hikashop_order_payment_title title">
					<?php // echo JHTML::_('grid.sort', JText::_('Metoda wysyłki'), 'b.order_shippid_method', $this->pageInfo->filter->order->dir,$this->pageInfo->filter->order->value ); ?>
				</th>
					...
					<td class="hikashop_order_payment_value">
						<?php if(!empty($row->order_payment_method)){
							if(!empty($this->payments[$row->order_payment_id])){
								...
								echo $row->order_shippid_method;
								
							}
							
						} 
						
						//$row->order_shipping_id; 
