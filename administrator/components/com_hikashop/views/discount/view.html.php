...
class DiscountViewDiscount extends hikashopView {
	...
	public function listing($extendedData = true) {
		...
		$searchMap = array('a.discount_code','a.discount_id','a.klient_email');
		$filters = array();
		if(!empty($pageInfo->search)){
			$searchVal = '\'%'.hikashop_getEscaped($pageInfo->search,true).'%\'';
			$filters[] = implode(" LIKE $searchVal OR ",$searchMap)." LIKE $searchVal";
		}
	if(isset($_POST['prefiks']) && $_POST['prefiks']!=''){
	$filters[] = "discount_code LIKE '".$_POST['prefiks']."%'";
	}
	if(isset($_POST['wykorzystany']) && $_POST['wykorzystany']=='0'){
	$filters[] = "discount_used_times = '' ";
	...
	if(empty($ids)) { } else {
			...
			$filters[] = 'discount_id IN ('.implode(',', $ids).')';
		}
	
	
	if(isset($_POST['wykorzystany']) && $_POST['wykorzystany']=='1'){
	$filters[] = "discount_used_times != ''";
