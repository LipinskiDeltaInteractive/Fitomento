...
class Route66ModelInstantArticlesFeeds extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->qn('#__route66_instant_articles_feeds'));
	...
	public function getItems()
	{
		...
		{
			$item->previewLink = $this->getPreviewLink('index.php?option=com_route66&view=instantarticlesfeed&id=' . $item->id . '&format=xml') . $suffix;
			$item->editLink = JRoute::_('index.php?option=com_route66&task=instantarticlesfeed.edit&id=' . $item->id);
		}
